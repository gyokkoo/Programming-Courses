...
const encryptyion = require('../utilities/encryption')
...
module.exports.registerPost = (req, res) => {
  ...
  if (user.username && user.password !== user.confirmedPassword) {
    user.error = 'Passwords do not match'
    res.render('user/register', user)
    return
  }

  let salt = encryptyion.generateSalt()
  user.salt = salt

  if (user.password) {
    let hashedPassword = encryptyion.generateHashedPassword(salt, user.password)
